...
class CandidatesTab:
    ...
    def backtest_selected(self):
        ...
        if not sym:
            messagebox.showinfo("Backtest", "Select a candidate first.")
            return

        # Prefer TradingApp helper if present
        if self.app and hasattr(self.app, "run_single_symbol_backtest_thread"):
            try:
                self.app.run_single_symbol_backtest_thread(sym)
                try:
                    self.app.tab_view.set("Backtest Lab")
                except Exception:
                    pass
                return
            except Exception:
        ...
        messagebox.showwarning("Backtest", "Backtest function unavailable in this build.")
